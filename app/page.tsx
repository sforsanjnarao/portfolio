...
export default function Portfolio() {
  ...
  return (
    <main className="bg-black text-zinc-200 font-sans min-h-screen w-full flex flex-col lg:flex-row selection:bg-purple-500/30">
      {/* <div className="fixed inset-0 z-50 pointer-events-none opacity-[0.1]" 
      ...
      <aside className="w-full lg:w-[28%] lg:h-screen lg:sticky lg:top-0 bg-black border-r border-zinc-800 p-6 flex flex-col gap-2 z-20">
        ...
        {/* 2. TECH ARSENAL (Separate Box for better spacing) */}
        ...
        <div className="flex flex-col gap-2 mt-auto">
            ...
            <div className="grid grid-cols-2 gap-2">
               ...
               <SocialBtn href="https://drive.google.com/file/d/1zICVAP9SsgSNvdVCUx6QM1aT68hxBJWR/view?usp=sharing" 
      ...
      <div className="w-full lg:w-[72%] h-screen overflow-y-auto px-6 lg:px-12 py-6 flex flex-col scroll-smooth">
        ...
        {/* HERO SECTION (Compact to fit Projects below) */}
      ...
      {activeVideo && (
        <div 
          ...
          onClick={() => setActiveVideo(null)} // Close when clicking background
        >
          <div 
          ...
          >
            ...
            <video 
              ...
              controls // Adds timeline, play/pause, volume
              autoPlay 
            />
          </div>
        </div>
...
function ProjectCard({ data,onPlay  }: { data: any, onPlay: (url: string) => void }) {
  // FIXED: Changed dark blue to Cyan so it is visible on black
  const accentColors: any = {
    purple: "group-hover:text-purple-400 border-purple-500/20",
    red: "group-hover:text-red-400 border-red-500/20",
    blue: "group-hover:text-cyan-400 border-cyan-500/20", // FIXED THIS COLOR
    ...
    <div className={`w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-5 flex flex-col md:flex-row gap-6 hover:border-opacity-100 transition-all group hover:bg-zinc-900/80 ${accentColors[data.theme].split(" ")[1]}`}>
       ...
       <div className="flex-1 flex flex-col justify-center pl-2">
          ...
          <div className="flex gap-3">
             <a href={data.link} className="flex items-center gap-2 px-5 py-2.5 bg-zinc-100 text-zinc-950 rounded-full text-xs font-bold hover:bg-white hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                <ExternalLink size={14} /> 
                Live Demo
              </a>

             <a href={data.repo} className="flex items-center gap-2 px-5 py-2.5 bg-zinc-950 border border-zinc-800 text-zinc-300 rounded-full text-xs font-bold hover:bg-zinc-900 hover:text-white hover:border-zinc-700 hover:scale-105 active:scale-95 transition-all">
                <Github size={14} /> 
                Code
              </a>
       ...
       {/* RIGHT: VIDEO/IMAGE PREVIEW 
           Optimized Height: 180px ensures 2 projects fit on screen
       */}
       {/* RIGHT: VIDEO PREVIEW */}
      <div 
        className="w-full md:w-[280px] h-[200px] bg-black rounded-2xl border border-zinc-800 relative overflow-hidden group-hover:shadow-2xl transition-all shadow-lg shadow-black/50"
        onClick={() => onPlay(data.video)} // Triggers the modal
      >
...
//   return (
//     <div 
//       className="aspect-square bg-black border-2 rounded-xl flex items-center justify-center cursor-default group relative overflow-visible hover:scale-110 hover:shadow-xl transition-all duration-300 hover:-translate-y-2"
//       style={{ borderColor: `${color}30` }}
//     >
//        <div 
//          className="group-hover:scale-125 transition-all duration-300" 
//          style={{ color: color, filter: `drop-shadow(0 0 10px ${color}60)` }}
//        >
//          {icon}
//        </div>
//        <span     
//          className="absolute -top-10 left-1/2 -translate-x-1/2 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-10 whitespace-nowrap shadow-lg"
